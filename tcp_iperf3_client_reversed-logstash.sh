...
0)  echo "[chprobe_iperf3] Let's see if we can start the tcp daemon" | logger -p info
    ...
    iw $iwnic station dump | egrep 'tx bitrate|signal' | xargs | tr -d 'short|GI' | sed 's/\<VHT-NSS\>//g' | sed -e "s/^/$direction /" | awk '{print $1,$3,$15,$18,$19,$20}' | tr -d 'MHz' | sed 's/$/vht/' | logger -t tx_linkstats_$phy[$(echo $count)] -p local3.debug && iw $iwnic station dump | egrep 'rx bitrate|signal' | xargs | tr -d 'short|GI' | sed 's/\<VHT-NSS\>//g' | sed -e "s/^/$direction /" | awk '{print $1,$3,$15,$18,$19,$20}' | tr -d 'MHz' | sed 's/$/vht/' | logger -t rx_linkstats_$phy[$(echo $count)] -p local3.debug && iw $iwnic station dump | egrep 'bytes|packets|retries|failed' | xargs | tr -d 'rx|tx|bytes|packets|retries|failed:' | tr -s ' ' | logger -t iw_counters[$(echo $count)] -p local3.debug
