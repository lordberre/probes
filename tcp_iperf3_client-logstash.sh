...
0)  echo "[chprobe_iperf3] Let's see if we can start the tcp daemon" | logger -p info
    ...
    iw $iwnic link | egrep 'bitrate|signal' | tr -d 'short|GI' | sed 's/\<VHT-NSS\>//g' | xargs | sed -e "s/^/$direction /" | awk '{print $1,$3,$7,$10,$11,$12,$13}' | tr -d 'MHz' | sed 's/$/vht/'| logger -t linkstats_$phy[$(echo $count)] -p local3.debug
