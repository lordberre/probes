...
0)  echo "[$logtag] Let's see if we can start the tcp daemon" | logger -p info
    ...
    /bin/iperf3 --client $target -4 -T $direction -P 15 -t 12 -O 2 2>&1 | egrep 'SUM.*rece' | awk '/Mbits\/sec/ {print $1,$7}' | tr -d ':' | logger -t iperf3tcp[$(echo $count)] -p $logfacility & echo "[$logtag] tcp daemon started" | logger -p info &
...
echo "[$logtag] Starting the tcp daemon - $direction" | logger -p info
    /bin/iperf3 --client $target -4 -T $direction -P 15 -t 12 -O 2 2>&1 | egrep 'SUM.*rece' | awk '/Mbits\/sec/ {print $1,$7}' | tr -d ':' | logger -t iperf3tcp[$(echo $count)] -p $logfacility & echo "[$logtag] Okey the daemon seems to be finished - starting our tcp daemon" | logger -p info &
        if [ $iwdetect -gt 0 ]; then
	    if [ $wififreq -lt 2500 ]; then phy=ht && eval $htparse;else
                    if [ $phydetect -ge 1 ]; then phy=vht && eval $vhtparse;else phy=ht eval $htparse;fi;fi
	    else echo 'No WiFi NIC detected'>/dev/stdout;fi      
