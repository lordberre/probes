...
0)  echo "[chprobe_iperf3] Let's see if we can start the tcp daemon" | logger -p info
    ...
    /bin/iperf3 --client $target -T $direction -P 15 -w 1m 2>&1 | egrep 'SUM.*rece' | awk '/Mbits\/sec/ {print $1,$7}' | tr -d ':' | logger -t iperf3tcp[$(echo $count)] -p local3.debug & echo "[chprobe_iperf3] tcp daemon started" | logger -p info &
...
1)  echo "[chprobe_iperf3] iperf tcp daemon is already running" | logger -p info
    ...
    echo "[chprobe_iperf3] Starting the tcp daemon - $direction" | logger -p info
    /bin/iperf3 --client $target -T $direction -P 15 -w 1m 2>&1 | egrep 'SUM.*rece' | awk '/Mbits\/sec/ {print $1,$7}' | tr -d ':' | logger -t iperf3tcp[$(echo $count)] -p local3.debug & echo "[chprobe_iperf3] Okey the daemon seems to be finished - starting our tcp daemon" | logger -p info &
        if [ $iwdetect -gt 0 ]; then
	    if [ $wififreq -lt 2500 ]; then phy=ht & eval $htparse;else
		    if [ $phy = vht ]; then eval $vhtparse;else eval $htparse;fi;fi
	    else echo 'No WiFi NIC detected'>/dev/stdout;fi      
