...
0)  echo "[chprobe_iperf3] iperf3 tcp daemon not running, initiating random timer then restarting daemon:" | logger -p info
    ...
    /bin/iperf3 --client $target -T Upload -P 15 -w 1m 2>&1 | egrep 'SUM.*rece' | awk '/Mbits\/sec/ {print $1,$7}' | logger -t iperf3tcp[$(echo $count)] -p local3.debug & echo "[chprobe_iperf3] Timer stopped - tcp daemon started" | logger -p info
     sleep 15
