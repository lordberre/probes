...
0)  echo "[chprobe_iperf3] Let's see if we can start the tcp daemon" > /dev/stdout
    while /usr/local/bin/iperf3 -c $target -t 1 | grep busy; do sleep $[ ( $RANDOM % 5 ) + 3]s  && echo '[chprobe_iperf3] waiting cuz server is busy' > /dev/stdout;done
    echo "[chprobe_iperf3] Starting the tcp daemon - $direction" > /dev/stdout
    /usr/local/bin/iperf3 --client $target -T $direction -P 15 -w 1m 2>&1 | egrep 'SUM.*rece' | awk '/Mbits\/sec/ {print $1,$7}' | tr -d ':' | eval $maclogformat2 >>$maclogdir & echo "[chprobe_iperf3] tcp daemon started" > /dev/stdout
            eval $run_airport | sed -e "s/^/$direction /" | if [ $linkformat -ge 1 ]; then eval $maclogformat;else eval $twentymhzformat;fi >>$maclogdir
    ;;
1)  echo "[chprobe_iperf3] iperf tcp daemon is already running" > /dev/stdout
    while pgrep -f "iperf3 --client" | wc -w | grep 1; do sleep $[ ( $RANDOM % 5 ) + 3]s && echo '[chprobe_iperf3] waiting cuz a daemon is running' > /dev/stdout;done
    echo "[chprobe_iperf3] Starting the tcp daemon - $direction" > /dev/stdout
    /usr/local/bin/iperf3 --client $target -T $direction -P 15 -w 1m 2>&1 | egrep 'SUM.*rece' | awk '/Mbits\/sec/ {print $1,$7}' | tr -d ':' | eval $maclogformat2 >>$maclogdir & echo "[chprobe_iperf3] Okey the daemon seems to be finished - starting our tcp daemon" > /dev/stdout
        eval $run_airport | sed -e "s/^/$direction /" | if [ $linkformat -ge 1 ]; then eval $maclogformat;else eval $twentymhzformat;fi >>$maclogdir
    ;;
*)  echo "[chprobe_iperf3] multiple instances of iperf udp daemon running. Stopping & restarting iperf:" > /dev/stdout
